        ...
        try
        {
            ...
            if (converter == null)
            {
                return new ConverterIdentificationResult
                {
                    ...
                    Message =
                        $"No converter found for type '{type.Name}'. System.Text.Json will use default serialization."
                };
            }
            ...
            return new ConverterIdentificationResult
            {
                ...
                Message = $"Using converter '{converterType.Name}' for type '{type.Name}'" +
                          (isToolkitConverter
                              ? $" (JsonToolkit.STJ converter with precedence {precedence})"
                              : " (System converter)")
            };
        }
        catch (Exception ex)
                ...
                if (toolkitConverters.Count > 1)
                {
                    ...
                    if (precedences.Count == 1)
                    {
                        ...
                        result.Conflicts.Add(new ConverterConflict
                        {
                            ...
                            Message =
                                $"Multiple JsonToolkit.STJ converters with same precedence ({precedences[0]}) for type '{type.Name}'"
                        });
                    }
                }
