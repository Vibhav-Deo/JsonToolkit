...
public class OptionalPropertyConverterFactory : JsonConverterFactory
{
    ...
    public OptionalPropertyConverterFactory RegisterDefaults<T>(OptionalPropertyDefaults<T> defaults)
        where T : class, new()
    {
        if (defaults == null)
    ...
    /// <returns>The current factory instance for method chaining.</returns>
    public OptionalPropertyConverterFactory RegisterDefaults<T>(Action<OptionalPropertyDefaults<T>> configure)
        where T : class, new()
    {
        if (configure == null)
    ...
    {
        return
            $"OptionalPropertyConverterFactory: {RegisteredTypesCount} registered types, {_converterCache.Count} cached converters";
    }
}