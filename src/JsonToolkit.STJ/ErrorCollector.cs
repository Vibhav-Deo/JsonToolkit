...
public class ErrorCollector
{
    ...
    public void AddValidationError(string message, string errorType = "ValidationError", object? attemptedValue = null,
    ...
    /// <param name="propertyPath">The property path (uses current path if null).</param>
    public void AddTypeConversionError(string message, Type expectedType, Type? actualType = null,
        object? attemptedValue = null, string? propertyPath = null)
    {
        var error = new CollectedError
        {
    ...
    /// <param name="propertyPath">The property path (uses current path if null).</param>
    public void AddConverterError(string message, string converterName, string? operation = null,
        Exception? innerException = null, string? propertyPath = null)
    {
        var error = new CollectedError
        {
    ...
    /// <param name="innerException">The inner exception that caused the error.</param>
    /// <param name="propertyPath">The property path (uses current path if null).</param>
    public void AddError(string message, string? operation = null, Exception? innerException = null,
        string? propertyPath = null)
    {
        var error = new CollectedError
        {
    ...
    public IEnumerable<CollectedError> GetValidationErrorsByType(string validationErrorType)
    {
        return _errors.Where(e => e.ErrorType == ErrorType.Validation &&
                                  string.Equals(e.ValidationErrorType, validationErrorType,
    ...
    public JsonValidationException CreateValidationException(string message = "JSON validation failed")
    {
        var validationErrors = _errors
            ...
            .Select(e =>
                new ValidationError(e.PropertyPath ?? "$", e.Message, e.ValidationErrorType ?? "ValidationError"))
            .ToList();
